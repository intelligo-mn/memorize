package cloud.techstar.memorize.utils;

public class MemorizeConstant {

//    private static String WEB_URL = "https://jisho.techstar.cloud/api/";
    public static String WEB_URL = "https://techstar-cloud-tortuvshin.c9users.io/api/";

    public static String GET_WORDS = WEB_URL.concat("words");
    public static String CREATE_WORD = WEB_URL.concat("word/create");
    public static String CREATE_MULTIPLE = WEB_URL.concat("multi/create");
    public static String EDIT_MULTIPLE = WEB_URL.concat("multi/edit");
}
